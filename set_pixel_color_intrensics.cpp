                       ...
                       float offs_re, float offs_im, float scale)
{
    ...
    for (int k = 0; k < BUF_SIZE; ++k) x_angle_arr[i] = x_angle_coef;
    for (int k = 0; k < BUF_SIZE; ++k) y_angle_arr[i] = y_angle_coef;
    for (int k = 0; k < BUF_SIZE; ++k) x_free_arr[i] = x_free_coef;
    for (int k = 0; k < BUF_SIZE; ++k) y_free_arr[i] = y_free_coef;
    ...
    for (int x = 0; x < COLS_NUM; ++x) {
        for (int y = 0; y < ROWS_NUM; y += BUF_SIZE) {
            ...
            for (int i = 0; i < BUF_SIZE; ++i)  y_arr[i] = x;
