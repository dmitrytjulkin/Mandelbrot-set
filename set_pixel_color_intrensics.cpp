                       ...
                       float offs_re, float offs_im, float scale)
{
    ...
    __m256 x_angle_vec[BUF_SIZE] = {};
    __m256 y_angle_vec[BUF_SIZE] = {};
    __m256 x_free_vec[BUF_SIZE]  = {};
    __m256 y_free_vec[BUF_SIZE]  = {};

    x_angle_vec = _mm256_set1_ps (x_angle_coef);
    y_angle_vec = _mm256_set1_ps (y_angle_vec);
    x_free_vec = _mm256_set1_ps (x_free_vec);
    y_free_vec = _mm256_set1_ps (y_free_vec);

    __m256i xi_vec[BUF_SIZE]  = {};
    __m256i yi_vec[BUF_SIZE]  = {};
    __m256i i_vec[BUF_SIZE]  = {};

    __m256 xf_vec[BUF_SIZE] = {};
    __m256 yf_vec[BUF_SIZE] = {};

    __m256 re0_vec[BUF_SIZE] = {};
    __m256 im0_vec[BUF_SIZE] = {};

    __m256 ren_vec[BUF_SIZE]  = {};
    __m256 imn_vec[BUF_SIZE]  = {};
    __m256 re2n_vec[BUF_SIZE] = {};
    __m256 im2n_vec[BUF_SIZE] = {};

    __m256  cmprbl_vec[BUF_SIZE]  = {};
    __m256i cmp_res_vec[BUF_SIZE] = {};

    __m256i n_vec[BUF_SIZE]      = {};
    __m256i mode_n_vec[BUF_SIZE] = {};
    ...
    for (int x = 0; x < COLS_NUM; ++x) {
        for (int y = 0; y < ROWS_NUM; y += BUF_SIZE) {
            ...
            re0_vec = _mm256_fmsub_ps (xi_vec, x_angle_vec, x_free_vec);
            im0_vec = _mm256_fmsub_ps (yi_vec, y_angle_vec, y_free_vec);
            ...
            cmp_res_vec = _mm256_setzero_ps ();

            n_vec      = _mm256_setzero_ps ();
            ...
            while (iteration < MAX_ITER_VAL){
                ...
                cmp_res_vec = _mm256_cmpgt_epi32 (_mm256_set1_ps (RADIUS_2), cmprbl_arr);        // RADIUS_2 > re2n + im2n
                cmp_res_vec = _mm256_and_ps (cmp_res_vec, _mm256_set1_epi32 (1));
                mode_n_vec  = _mm256_mul_ps (cmp_res_vec, mode_n_vec);          // mode_n_vec[i] = 1 if true, 0 otherwise
                n_vec       = _mm256_add_ps (n_vec, mode_n_vec);                // n_i += mode_n_vec[i]
