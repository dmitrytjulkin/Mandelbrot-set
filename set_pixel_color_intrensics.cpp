                       ...
                       float offs_re, float offs_im, float scale)
{
    ...
    for (int x = 0; x < COLS_NUM; ++x) {
        for (int y = 0; y < ROWS_NUM; y += BUF_SIZE) {
            ...
            re0_vec = _mm256_fmsub_ps (_mm256_set1_ps (x), x_angle_vec, x_free_vec);
            im0_vec = _mm256_fmsub_ps (_mm256_set1_ps (y), y_angle_vec, y_free_vec);
...
void ColorPixels (sf::VertexArray* pixels, __m256i x_vec, __m256i y_vec, __m256i n_vec)
{
    ...
    _mm256_store_si256 ((__m256i*) x_arr, x_vec);
    _mm256_store_si256 ((__m256i*) y_arr, y_vec);
